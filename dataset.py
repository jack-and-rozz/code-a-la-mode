...
def parse_rewards_sum(log_strs):
  ...
  rewards = [_scores_to_rewards(scores[i:i+200]) for i in range(0, 600, 200)] # tuple of reward lists in each game.
  return rewards
...
class Dataset():
  def __init__(self, log_files, batch_size):
    ...
    self.data = [parse_log(file_path) for i, file_path in enumerate(log_files) if -1 not in self.scores[i]] # 4 * num_games (player1-round1, player2-round1, player1-round2, player2-round3)
    self.scores = [s for s in self.scores if -1 not in s]
    idxs = list(range(len(self.data)))
    random.shuffle(idxs)
    self.data = flatten([self.data[i] for i in idxs])

    self.average_score = int(sum([s[0] for s in self.scores] + [s[1] for s in self.scores])/len(self.scores)/2)
